...
from humanfriendly import format_timespan, parse_size, parse_timespan, format_size
...
class BackupSim:
    ...
    def __init__(self, max_t, nodes):
        ...
        info_dict = {
            ...
            "average_downtime": (nodes[0].average_downtime, format_timespan),
            "average_recover_time": (nodes[0].average_recover_time, format_timespan),
            "average_lifetime": (nodes[0].average_lifetime, format_timespan)
        }
        ...
        text2 = ""
        t1 = False
        for k, (v, func) in info_dict.items():
            val = func(v) if func is not None else v
            if t1:
                text = text + k + " = " + str(val) + "\n"
            else:
                text2 = text2 + k + " = " + str(val) + "\n"
            t1 = not t1
        ...
        plt.gcf().text(0.70, 0.90, text, fontsize=8, bbox=props)
