...
class Node:
    ...
    def schedule_next_upload(self, sim: Backup):
        ...
        for peer, block_id in self.remote_blocks_held.items():
            # if the block is not present locally and the peer is online and has bandwidth available
            # to download something, then schedule the restore from self to peer of block_id
            if peer.online and peer.available_bw_download > 0 and not peer.local_blocks[block_id]:
        ...
        for peer in sim.nodes:
            # if the peer is not self, is online, is not among the remote owners, has enough space and has
            # available bandwidth to download something, schedule the backup of block_id from self to peer
            if (peer is not self and peer.online and peer not in remote_owners and peer.available_bw_download > 0
    ...
    def schedule_next_download(self, sim: Backup):
        ...
        if self.available_bw_download == 0:
            return False

        # first find if we have a missing block to restore
        for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
            # if i don't have one of my blocks locally, but a peer does, if it is online
            # and has available bandwidth to upload something, then schedule the restore from peer to self
            if not held_locally and peer is not None and peer.online and peer.available_bw_upload > 0.0:
        ...
        for peer in sim.nodes:
            # if the peer is not self and it is online and it has available bandwidth to upload something
            ...
            # to peer
            if (peer is not self and peer.online and peer.available_bw_upload > 0 and peer not in
...
class NodeEvent(Event):
    ...
    node: Node  # node target of the event
...
class Fail(Disconnection):
    ...
    def process(self, sim: Backup):
        ...
        for owner, block_id in node.remote_blocks_held.items():
            ...
            # and has available bandwidth to upload something
            # then try to upload the block somewhere else
            if owner.online and owner.available_bw_upload > 0:
