...
class MMN(Simulation):
    def __init__(self, lambd, mu, n, shape):
        ...
        if shape > 0:
            self.arrival_gen = weibull_generator(shape, 1 / (n * lambd))
            self.completion_gen = weibull_generator(shape, 1 / mu)
        else:
...
class MMNImpl(MMN):
    ...
    def simulate(self, max_t, shape, file):
        self.run(max_t)
...
class MMNRoundRobinSim:
    def __init__(self, lambd, mu, n, d, max_t, shape, file):
        ...
        self.quantums = sorted(self.quantums)
    ...
    def plot_time_quantum(self, quantum_times, title):
        ...
        plt.plot(quantums, avg_times, label="Round Robin scheduling")
        plt.title(title)
...
class ArrivalRR(Event):
    ...
    def process(self, sim: MMNRoundRobin):
        ...
        if sim.supermarket_mode:
            queue_to_ins = sim.supermarket_decision()
        else:
            queue_to_ins = randrange(sim.n)
